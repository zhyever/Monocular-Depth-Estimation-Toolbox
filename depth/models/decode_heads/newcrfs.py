...
class NewCRFHead(DepthBaseDecodeHead):
    """DenseDepthHead.
    ...
    def __init__(self,
                 ...
                 v_dims,
                 **kwargs):
        super(NewCRFHead, self).__init__(**kwargs)

        self.crf3 = NewCRF(input_dim=self.in_channels[3], embed_dim=crf_dims[3], window_size=window_size, v_dim=v_dims[3], num_heads=32)
        self.crf2 = NewCRF(input_dim=self.in_channels[2], embed_dim=crf_dims[2], window_size=window_size, v_dim=v_dims[2], num_heads=16)
        self.crf1 = NewCRF(input_dim=self.in_channels[1], embed_dim=crf_dims[1], window_size=window_size, v_dim=v_dims[1], num_heads=8)
        self.crf0 = NewCRF(input_dim=self.in_channels[0], embed_dim=crf_dims[0], window_size=window_size, v_dim=v_dims[0], num_heads=4)


    def forward(self, inputs, img_metas):
        """Forward function."""

        # default NewCRF: swin1(0), swin2(1), swin3(2), swin4(3), ppmout(-1)
        e3 = self.crf3(inputs[3], inputs[-1])
        e3 = nn.PixelShuffle(2)(e3)
        e2 = self.crf2(inputs[2], e3)
        e2 = nn.PixelShuffle(2)(e2)
        e1 = self.crf1(inputs[1], e2)
        e1 = nn.PixelShuffle(2)(e1)
        e0 = self.crf0(inputs[0], e1)
        output = self.depth_pred(e0)
